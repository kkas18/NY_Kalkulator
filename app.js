(() => {
  ...
  function render(){
    ...
    result.textContent=value!==null?fmt(value):expr?pretty(expr):'0';
  ...
  function perform(action){
    switch(action){
      ...
      case 'angle':settings.angle=settings.angle==='DEG'?'RAD':'DEG';$('#angleSelect').value=settings.angle;$('#angleButton').textContent=settings.angle;store();break;
  ...
  function syncSettings(){document.documentElement.dataset.theme=settings.theme;document.querySelector('meta[name="theme-color"]').content=settings.theme==='light'?'#eeeeec':'#090a0b';$('#themeSelect').value=settings.theme;$('#decimalSelect').value=settings.decimal;$('#angleSelect').value=settings.angle;$('#angleButton').textContent=settings.angle;$('#decimalButton').textContent=settings.decimal;$('#vibrationCheck').checked=settings.vibration;$('#soundCheck').checked=settings.sound;render()}
  ...
  $('#themeSelect').addEventListener('change',e=>{settings.theme=e.target.value;store();syncSettings()});
  $('#decimalSelect').addEventListener('change',e=>{settings.decimal=e.target.value;store();syncSettings();renderHistory()});
  $('#angleSelect').addEventListener('change',e=>{settings.angle=e.target.value;store();syncSettings()});
  $('#vibrationCheck').addEventListener('change',e=>{settings.vibration=e.target.checked;store()});
  $('#soundCheck').addEventListener('change',e=>{settings.sound=e.target.checked;store()});
